...
class StructuredPruningExperiment(TrainingExperiment):
    def __init__(self,
                 ...
                 strategy,
                 compressions,
                 fullnet_comp=True, # if True input compression applies to full net, otherwise it applies to prunable layers only
        ...
        if np.isscalar(compressions):
            compressions = [compressions]
        self.add_params(strategy=strategy, compressions=compressions, reweight=reweight, bias=bias, structure=structure,
        ...
        self.build_pruning(strategy, compressions, fullnet_comp, reweight, bias, structure, prune_layers, nbatches, **prune_kwargs)
        self.verif = verif
        self.path = path
    ...
    def build_pruning(self, strategy, compressions, fullnet_comp, reweight, bias, structure, prune_layers, nbatches, **prune_kwargs):
        if self.pruned_path is None:
            ...
            self.pruning = constructor(self.model, xs, ys, compressions=compressions, fullnet_comp=fullnet_comp,
                                       reweight=reweight, bias=bias, structure=structure, prune_layers=prune_layers, **prune_kwargs)
            self.pruning_time = (time.perf_counter() - since)/len(compressions)  # assuming all compressions required similar time..

    def run(self):
        for compression in self.params['compressions']:
            child = copy.deepcopy(self) if len(self.params['compressions']) > 1 else self
            del child.params['compressions']
            child.add_params(compression=compression)
            if self.pruned_path is None:
                since = time.perf_counter()
                child.pruning.apply(compression)
                child.pruning_time += time.perf_counter() - since
                printc(f"Masked model with compression={compression}", color='GREEN')
                # printc(f"Masked model with fraction={fraction} of prunable channels kept", color='GREEN')
            else:
                model_state = torch.load(f'{self.pruned_path}/checkpoints/checkpoint--1.pt',
                ...
                printc(f"Loaded masked model with compression={compression}", color='GREEN')
            ...
            if self.pruned_path is None:
                ...
                self.log_epoch(-1)
            if compression > 1 or self.params['finetune']:  # fraction < 1
                child.run_epochs()
    ...
    def save_metrics(self):
        ...
        summary = self.pruning.summary(self.params['compression'])
    ...
    def pruning_metrics(self):
        ...
        metrics['size'] = size
        metrics['size_nz_orig'] = self.pruning.size_nz_orig
        metrics['size_nz'] = size_nz
        metrics['compression_ratio'] = size / size_nz # self.size_nz_orig
        ...
        metrics['theoretical_speedup'] = ops / ops_nz # self.ops_nz_orig
