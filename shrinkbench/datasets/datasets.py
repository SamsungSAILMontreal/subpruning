...
def dataset_builder(dataset, train=True, normalize=None, preproc=None, path=None):
    ...
    return _constructors[dataset](path, download=True, **kwargs)


def MNIST(train=True, path=None, cifar_shape=False):
    """Thin wrapper around torchvision.datasets.CIFAR10
    """
    mean, std = 0.1307, 0.3081
    normalize = transforms.Normalize(mean=(mean,), std=(std,))
    preproc = [transforms.Resize((32, 32)), transforms.Lambda(lambda img:  img.convert('RGB'))] if cifar_shape else []
    dataset = dataset_builder('MNIST', train, normalize, preproc, path)
    dataset.shape = (3, 32, 32) if cifar_shape else (1, 28, 28)
    return dataset
