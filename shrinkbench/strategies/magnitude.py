...
class GlobalMagWeight(VisionPruning):
    # prunes the weights with the lowest absolute value anywhere in the network
...
class LayerMagWeight(LayerPruning, VisionPruning):
    # for each layer, prunes the weights with the lowest absolute value
...
class GlobalMagGrad(GradientMixin, VisionPruning):
    # prunes the weights with the lowest absolute value of (weight x gradient of loss w.r.t weight param),
    # evaluated on a batch of inputs.
    def model_masks(self):
        params = self.params()
...
class LayerMagGrad(GradientMixin, LayerPruning, VisionPruning):
    # for each layer, prunes the weights the lowest absolute value of (weight x gradient),
    # evaluated on a batch of inputs
    def layer_masks(self, module):
        params = self.module_params(module)
...
class GlobalMagAct(ActivationMixin, VisionPruning):
    # prunes the weights with the lowest absolute value of (weight x l1 norm of mean activations on a batch of inputs)
    def model_masks(self):
        params = self.params()
