...
template <class T>
class Random : public SubmodularOptimizer<T> {
protected:
    unsigned int cnt = 0;
...
public:
     ...
     * @param f The function which should be maximized. Note, that the `clone' function is used to construct a new SubmodularFunction<T> which is owned by this object. If you implement a custom SubmodularFunction make sure that everything you need is actually cloned / copied.
     * @param seed The random seed used for randomization.
     */
    Random(unsigned int K, SubmodularFunction<T> & f, unsigned long seed = 0) : SubmodularOptimizer<T>(K,f), generator(seed) {}
     ...
     * @param seed The random seed used for randomization.
     */
    Random(unsigned int K, std::function<data_t (std::vector<T> const &)> f, unsigned long seed = 0) : SubmodularOptimizer<T>(K,f), generator(seed) {}
    ...
    void fit(std::vector<T> const & X, std::vector<idx_t> const & ids, unsigned int iterations = 1) {
        ...
        std::vector<unsigned int> indices = sample_without_replacement(this->K, X.size(), generator);

        for (auto i : indices) {
            this->f->update(this->solution, X[i], this->solution.size());
            this->solution.push_back(X[i]);
            if (ids.size() >= i) {
    ...
    void next(T const &x, std::optional<idx_t> const id = std::nullopt) {
        if (this->solution.size() < this->K) {
            ...
            if (j <= this->K) {
                this->f->update(this->solution, x, j - 1);
                if (id.has_value()) this->ids[j-1] = id.value();
                this->solution[j - 1] = x;
            }
        }
