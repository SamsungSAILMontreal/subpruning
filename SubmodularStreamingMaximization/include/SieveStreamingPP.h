...
*/
template <class T>
class SieveStreamingPP : public SubmodularOptimizer<T> {
private:
...
public:
    ...
    SieveStreamingPP(unsigned int K, SubmodularFunction<T> & f, data_t m, data_t epsilon)
        : SubmodularOptimizer<T>(K,f), lower_bound(0), m(m), epsilon(epsilon) {
    ...
    SieveStreamingPP(unsigned int K, std::function<data_t (std::vector<T> const &)> f, data_t m, data_t epsilon)
        : SubmodularOptimizer<T>(K,f), lower_bound(0), m(m), epsilon(epsilon) {
            // std::vector<data_t> ts = thresholds(m/(1.0 + epsilon), K * m, epsilon);
    ...
    void next(T const &x, std::optional<idx_t> const id = std::nullopt) {
        if (lower_bound != this->fval || sieves.size() == 0) {
            lower_bound = this->fval;
            data_t tau_min = std::max(lower_bound, m) / static_cast<data_t>(2.0*this->K);
            ...
            if (no_sieves_before > sieves.size() || no_sieves_before == 0) {
                std::vector<data_t> ts = thresholds(tau_min/(1.0 + epsilon), this->K * m, epsilon);
                ...
                for (auto t : ts) {
                    ...
                    if (!any) {
                        sieves.push_back(std::make_unique<Sieve>(this->K, *this->f, t));
                    }
                }
            }
        }
